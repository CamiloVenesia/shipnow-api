...
export const mocksService = {
    ...
    async generateAndPersistData({ users, orders, deliveries }) {
        ...
        try {
            ...
            const createdUsers = await mocksRepository.insertUsers(usersToCreate)
            ...
            for (let i = 0; i < ordersCount; i += 1) {
                ...
                const createdOrder = await mocksRepository.createOrder(orderData)
            ...
            for (let i = 0; i < deliveriesCount; i += 1) {
                ...
                const createdDelivery = await mocksRepository.createDelivery(deliveryData)
                createdDeliveries.push(createdDelivery)

                await mocksRepository.updateOrderDelivery(order._id, createdDelivery._id)
            }
